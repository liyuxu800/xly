...
void Dial_Motor_Control(dialStructDef *dialStruct)
{
  ...
  dialStruct->outerFeedback = (dialStruct->Angle + (float)angle_update_dial.finally_angle);
  dialStruct->innerFeedback = dialStruct->Speed;

  if (RC_CtrlData.rc.s2 == 2 & flag == 2)
  {
    flag = 0;
  }
  if (RC_CtrlData.rc.s2 == 3 & flag == 0)
  {
    flag = 1;
    if (RC_CtrlData.rc.s2 == 3 & flag == 1)
    {
