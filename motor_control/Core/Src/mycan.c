...
CAN_RxHeaderTypeDef rceStu_can1_fifo0;
uint8_t Data_can1_fifo0[8];
CAN_RxHeaderTypeDef rceStu_can2_fifo0;
uint8_t Data_can2_fifo0[8];
...
void CAN2_Transmit(uint32_t ID, uint8_t Length, uint8_t *Data) // can发送函数
{
	CAN_TxHeaderTypeDef TxMessage = {0};
	uint8_t Tx_Buffer[8] = {0};
	uint32_t box = 0;
...
{
	uint8_t data[8] = {0};
	if (HAL_CAN_GetRxFifoFillLevel(&hcan2, CAN_RX_FIFO0)) // 判断FIFO中是否有数据
	{
		if (HAL_CAN_GetRxMessage(&hcan2, CAN_RX_FIFO0, rceStu, data) == HAL_OK) // 如果接收到数据
		{
			for (uint8_t i = 0; i < 8; i++)
			{
				Data[i] = data[i];
